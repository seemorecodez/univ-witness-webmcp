...
assert(page.includes('One manifest. Two real runtimes. One controlled handoff.'), 'universal-deployment framing is missing');
assert(page.includes('browser-wasi') && page.includes('sites-edge-wasi'), 'both explicit targets must be visible');
assert(page.includes('Portability comparison'), 'visible portability comparison is missing');
...
for (const tool of ['get_univ_capabilities', 'plan_univ_deployment', 'create_deployment_handoff', 'deploy_univ_manifest', 'get_deployment_evidence']) {
...
console.log('UNIV Deploy verification passed: manifests, two real targets, handoff integrity, pinned artifacts, portability receipts, and evidence limits are intact.');
